...
class PipelineSuite
  ...
  test("pipeline fit events") {
    ...
    try {
      val pipelineModel = pipeline.fit(dataset0)
      val expected =
        FitPreEvent(pipeline, dataset0) ::
        FitEvent(pipeline, pipelineModel) :: Nil
  ...
  test("pipeline model transform tracker") {
    ...
    try {
      ...
      val expected =
        TransformPreEvent(newPipelineModel, dataset) ::
        TransformEvent(newPipelineModel, output) :: Nil
      eventually(timeout(10 seconds), interval(1 second)) {
        assert(expected === actual)
      }
  ...
  test("pipeline read/write events") {
    withTempDir { dir =>
      ...
      val listener = new SparkListener {
        override def onOtherEvent(event: SparkListenerEvent): Unit = event match {
          case e: SaveInstancePreEvent if e.path.endsWith("pipeline") => actual.append(e)
          case e: SaveInstanceEvent if e.path.endsWith("pipeline") => actual.append(e)
          case _ =>
        }
      }
      ...
      try {
        ...
        val expected = SaveInstancePreEvent(pipelineWriter, path) ::
          SaveInstanceEvent(pipelineWriter, path) :: Nil
        eventually(timeout(10 seconds), interval(1 second)) {
          assert(expected === actual)
        }
      } finally {
