  ...
  private[ml] object SharedReadWrite {
    ...
    def saveImpl(
      ...
      stages.zipWithIndex.foreach { case (stage, idx) =>
        ...
        MLEvents.withSaveInstanceEvent(mlWriter, path) {
          mlWriter.save(getStagePath(stage.uid, idx, stages.length, stagesDir))
        }
      }
    }
    ...
    def load(
      ...
      val stages: Array[PipelineStage] = stageUids.zipWithIndex.map { case (stageUid, idx) =>
        ...
        val reader = DefaultParamsReader.loadInstanceReader[PipelineStage](stagePath, sc)
        MLEvents.withLoadInstanceEvent(reader, path)(reader.load(path))
      }
      (metadata.uid, stages)
    }
...
class PipelineModel private[ml] (
  ...
  override def transform(dataset: Dataset[_]): DataFrame = MLEvents.withTransformEvent(
    ...
    stages.foldLeft(dataset.toDF)((cur, transformer) =>
      MLEvents.withTransformEvent(transformer, dataset)(transformer.transform(cur)))
  }
